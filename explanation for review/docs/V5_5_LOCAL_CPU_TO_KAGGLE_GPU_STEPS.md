# V5.5 Local CPU Prep To Kaggle GPU Docking

This handoff starts after local CPU prep has already finished.

Prepared locally:

- Generated candidates: `1113`
- SDF ligands prepared: `1036`
- Ligand PDBQT prepared: `1036`
- Vina-GPU-clean PDBQT ligands: `1036`
- ADMET/synthesis proxy rows: `1113`
- Docking grid: `06_docking/v5_5_pocket_guided/receptor/docking_grid_5AEP_QUP.json`

Kaggle should now skip SDF/PDBQT/sanitization and start from Vina-GPU.

## Cell 1: Copy Mounted Dataset To Working Directory

Change `INPUT_BASE` only if your Kaggle mounted path is different.
...
INPUT_BASE = Path("/kaggle/input/datasets/sriramnetwork/emd-v5-2-hybrid-v5-5-cpu-prepped/EMD_V5_2_Hybrid")
...
## Cell 2: Install GPU Runtime Dependencies

No `rdkit-pypi`. Use current `rdkit`.

```python
!apt-get update -qq
!apt-get install -y -qq git build-essential libboost-all-dev ocl-icd-opencl-dev opencl-headers clinfo
!python -m pip install -q --upgrade pip setuptools wheel
!python -m pip install -q pandas numpy pyyaml matplotlib pytest rdkit meeko
```
...
## Cell 3: Define Paths And Verify CPU-Prepped Inputs
...
for path in [SCORES_DIR, POSE_DIR, RAW_POSE_DIR, IMG_DIR]:
...
print("Candidates:", len(pd.read_csv(GEN_CSV)))
print("SDF count:", len(list(SDF_DIR.glob("CAND_*.sdf"))))
print("PDBQT count:", len(list(PDBQT_DIR.glob("CAND_*.pdbqt"))))
print("Clean Vina-GPU ligand count:", len(list(CLEAN_PDBQT_DIR.glob("CAND_*.pdbqt"))))
print("ADMET rows:", len(pd.read_csv(ADMET_CSV)))
print("Grid:", json.loads(GRID_JSON.read_text(encoding="utf-8")))
```

Stop here if the clean ligand count is `0`. Expected count is `1036`.

## Cell 4: Build Vina-GPU 2.1
...
if not BINARY.exists():
    shutil.rmtree(VINA_REPO, ignore_errors=True)
    subprocess.run(
...
def find_raw_pose(candidate_id: str, all_raw: list[Path]):
    exact_names = {f"{candidate_id}.pdbqt", f"{candidate_id}_out.pdbqt", f"{candidate_id}_vina_out.pdbqt"}
    for path in all_raw:
...
## Cell 7: Ranking, Pose, Pocket Scoring, SE3, Benchmark

ADMET is already included from the local CPU run, so this cell does not rerun ADMET.
...
!python scripts/07_rank_candidates.py \
  ...
  --admet-csv 07_admet_synthesis/v5_5_pocket_guided_admet_scores.csv \
  --output-csv 08_final_ranking/v5_5_pocket_guided_ranked_candidates.csv \
  --stage V5_5_gpu_ranking_with_pose
...
!python scripts/21_score_pocket_electronics.py \
  ...
  --top-n 0 \
  --stage V5_5_gpu_pocket_electronic_fit
...
!python scripts/23_score_se3_geometry.py \
  ...
  --device cuda \
  --num-repeats 3 \
  --stage V5_5_gpu_se3_geometry_scoring
...
## Cell 9: Zip Kaggle Outputs

```python
import shutil
from pathlib import Path

OUT_ROOT = Path("/kaggle/working/v5_5_gpu_outputs")
...
zip_path = shutil.make_archive("/kaggle/working/v5_5_gpu_outputs", "zip", OUT_ROOT)
print(zip_path)
```

Download and send back:

`/kaggle/working/v5_5_gpu_outputs.zip`
